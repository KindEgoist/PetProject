...
Версия 5.1 - Убрал все наброски с Кафкой. Было решено использовать Кафку как посредник(брокер) между сервисами.
Изменения:
- Изменения коснулись ConfigServer, EurekaServer и Gateway:
    * В Gateway убрал webflux и заменен на gateway
    * В ConfigServer и Gateway изменены ЯМЛ файлы для докера.
    * Во все три было добавлено Dockerfile и .dockerignore. Для создания докер образов. 
- Изменен docker-compose.yml под Кафку, ConfigServer, EurekaServer и Gateway
- В Store созданы классы Продюсер, Консюмер и Классы событий(Запрос и Ответ)
- В ProductInfo создал класс Consumer который мониторит Кафку из топика product-info-requests на наличии сообщений 
и получает его, дальше обрабаотывается и отправляется сообщение в product-info-responses топик в Кафку где его слушает 
Store.

Версия 5.2 - Соединение микросервисов Reserve и Store через Кафку.  
  -Реализован паттерн Outbox.  
  -Создана связь At Least Once + индопендетность.
